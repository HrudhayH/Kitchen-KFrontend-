...
export default function CartPage() {
  const { items, loading, count, removeItem, updateQty, clearCart } = useCart();
  ...
  if (!items || items.length === 0)
    return (
      <div className="max-w-4xl mx-auto p-6">
        ...
        <p className="mt-4">Your cart is empty. <Link href="/">Go shopping</Link></p>
      </div>
    );
      ...
      <div className="space-y-4 mt-4">
        {items.map((it) => (
          <div key={it.id} className="flex gap-4 items-center border p-4 rounded">
            <img src={it.product_snapshot.image_url || "/placeholder.png"} alt={it.product_snapshot.name} className="w-20 h-20 object-contain"/>
            <div className="flex-1">
              <div className="font-semibold">{it.product_snapshot.name}</div>
              <div className="text-gray-600">₹{it.product_snapshot.price} x {it.qty}</div>
              <div className="flex gap-2 mt-2">
                <button onClick={() => updateQty(it.id, Math.max(1, it.qty - 1))} className="px-2 py-1 border rounded">-</button>
