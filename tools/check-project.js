  ...
  files.forEach((filePath) => {
    ...
    try {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        if (
          matches.filter((m) => m.file === path.relative(repoRoot, filePath))
            .length >= 3
        ) {
          break;
        }
...
console.log(
  `✓ Profile components: ${report.frontend.profileComponents.status}`,
);
...
report.frontend.typescriptConfig = checkFiles(
  allFiles,
  ["kk-frontend/tsconfig\\.json$", "kk-frontend/next-env\\.d\\.ts$"],
...
keywords.forEach(({ keyword, extensions }) => {
  ...
  console.log(
    `  ${keyword.padEnd(30)} → ${result.count} occurrences in ${result.files.length} files`,
  );
});
...
console.log(
  `✓ Found: ${report.summary.found} (${Math.round((report.summary.found / report.summary.totalChecks) * 100)}%)`,
);
console.log(
  `✗ Missing: ${report.summary.missing} (${Math.round((report.summary.missing / report.summary.totalChecks) * 100)}%)`,
);
...
if (report.frontend.dashboardLayout.status === "missing") {
  report.suggestions.push(
    "Create DashboardLayout component for user account pages",
  );
...
if (report.frontend.authPages.paths.length === 0) {
  report.suggestions.push(
    "Create app/account/orders/page.tsx for order history",
  );
...
if (report.backend.services.status === "missing") {
  report.suggestions.push(
    "Implement business logic services for orders and products",
  );
...
if (report.backend.envFiles.status === "missing") {
  report.suggestions.push(
    "Create .env.example file in backend with required variables",
  );
  console.log(
    "⚠️  Create .env.example file in backend with required variables",
  );
}
...
if (report.suggestions.length === 0) {
  ...
  report.suggestions.push(
    "All critical features detected. Project looks ready for demo.",
  );
}
